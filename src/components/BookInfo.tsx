...
export const BookInfo = ({ item }: BookInfoProps): JSX.Element => {
  ...
  const imageUrl =
    typeof item.volumeInfo.imageLinks === "undefined" ? "images/noimage.png" : item.volumeInfo.imageLinks.smallThumbnail
  ...
  return (
    <ListItem>
      <Stack direction="row" spacing={2}>
        ...
        <ListItemText
          primary={
            <Link href={item.volumeInfo.previewLink} target="_blank" rel="noreferrer" sx={{ fontSize: "14px" }}>
              {title}
            </Link>
          }
          secondary={
